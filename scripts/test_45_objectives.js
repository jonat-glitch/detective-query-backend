...
// Define 3 progressive objectives for each of the 15 cases
const ALL_OBJECTIVES = [
  // ─── 30029: The Full Roster ───
  {
    ...
    objectives: [
      ...
      {
        order: 3,
        text: 'Identify staff and janitors who have criminal records (role IN ("Janitor", "Staff") AND criminal_record = 1). Note George Ramos (Janitor) and Oscar Villanueva.',
  ...
  // ─── 30030: Criminal Record Scan ───
  {
    ...
    objectives: [
      ...
      {
        order: 3,
        text: 'Pinpoint the only flagged Janitor on campus who holds master keys to the warehouse (role = "Janitor" AND criminal_record = 1). It points directly to George Ramos!',
  ...
  // ─── 30031: Young Suspect Filter ───
  {
    ...
    objectives: [
      ...
      {
        order: 3,
        text: 'Pinpoint the young student with a prior criminal record linked to campus thefts (role = "Student", age < 25, criminal_record = 1) ordered by name. Reveals Ethan Cruz and Lara Flores!',
  ...
  // ─── 30034: Evidence Per Location ───
  {
    ...
    objectives: [
      ...
      {
        order: 3,
        text: 'Pinpoint the single location with the highest evidence count having more than 2 evidence items. Confirms Warehouse is the primary crime scene!',
...
async function testAll() {
  ...
  console.log('Connected to playground DB. Testing all 45 queries...\n');

  let failed = 0;
  for (const c of ALL_OBJECTIVES) {
    console.log(`Checking Case ${c.case_id}: ${c.title}...`);
    for (const obj of c.objectives) {
      try {
        const [rows] = await conn.query(obj.query);
        if (!rows || rows.length === 0) {
          console.warn(`  ⚠️ Obj ${obj.order} returned 0 rows! Query: ${obj.query}`);
          failed++;
        } else {
          console.log(`  ✅ Obj ${obj.order} (${rows.length} rows)`);
        }
      } catch (err) {
        console.error(`  ❌ Obj ${obj.order} ERROR: ${err.message}\n  Query: ${obj.query}`);
        failed++;
      }
    }
  }
  ...
  await conn.end();
  console.log(`\nTest finished. Total failures: ${failed}`);
}
